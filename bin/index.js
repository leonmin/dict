...
const dict = async (argv, type) => {
    ...
    let { list = [], config = {} } = db.data;
    switch (type) {
        case 'query':
            if (argv._.length > 0) {
                dict = await query(argv._.join(' '));
                console.log(boxen(chalk.green(describeResult(dict)), { borderColor: 'green' }));
            ...
            if (!key && !value) {
                ...
                if (value) {
                    if (key === 'enabled' && [1, 2].includes(Number(value))) {
                        ;
                        db.data.config.enabled = Number(value) == 1 ? 1 : 2;
                        db.write();
                        console.log(chalk.green(`设置是否开启本地存储1开启2关闭: ${Number(value) == 1 ? 1 : 2}`));
                    }
                    else if (key === 'max') {
                        ;
                        db.data.config.max = Number(value) || configMaps.max;
                        db.write();
                        console.log(chalk.green(`设置查询列表最大数量: ${Number(value) || configMaps.max}`));
                    }
                }
                else {
...
yargs(hideBin(process.argv))
    ...
    .command(['config [key] [value]'], '获取/设置配置, max:最大存储数量 enabled:1开启2关闭', {}, (argv) => {
    dict(argv, 'config');
})
